...
export default function Contact() {
  ...
  return (
    <div>
      <section className="border-b border-[var(--line)]" style={{ backgroundImage: "radial-gradient(circle at 90% -20%, #F6E7DF, transparent 45%)" }}>
        <div className={SECTION + " py-16 md:py-20"}>
          <span className="text-[12px] font-bold uppercase tracking-widest text-[var(--clay)]">Contact us</span>
          <h1 className="font-news text-[36px] md:text-[46px] font-semibold tracking-tight mt-2">Let's talk newsrooms</h1>
          <p className="text-[var(--ink-soft)] text-[17px] mt-3 max-w-xl leading-relaxed">
            ...
            {INFO.map((i) => {
              ...
              return (
                <div key={i.t} className="flex gap-4">
                  <div className="w-11 h-11 shrink-0 rounded-xl bg-[var(--clay-tint)] text-[var(--clay-700)] grid place-items-center">
                    <Icon size={19} />
                  </div>
                  <div>
                    <div className="text-[11px] uppercase tracking-wide font-semibold text-[var(--muted)]">{i.t}</div>
                    <div className="font-semibold text-[15px] mt-0.5">{i.d}</div>
                    <div className="text-[13px] text-[var(--ink-soft)] mt-0.5">{i.s}</div>
                  </div>
                </div>
              );
            })}
          </div>
          <div className="mt-8 rounded-2xl border border-[var(--line)] bg-[var(--surface)] p-5 flex items-start gap-3">
            <Clock size={18} className="text-[var(--green)] mt-0.5 shrink-0" />
            <p className="text-[13.5px] text-[var(--ink-soft)] leading-relaxed">
              Working on a breaking story and need help fast? Mention <span className="font-semibold text-[var(--ink)]">“urgent”</span> in your message and we'll prioritise it.
            </p>
          </div>
        </div>

        {/* form */}
        <div className="rounded-2xl border border-[var(--line)] bg-[var(--surface)] p-6 md:p-8">
          {sent ? (
            <div className="h-full grid place-items-center text-center py-10 animate-fade-up">
              <div>
                <div className="w-14 h-14 rounded-full bg-[var(--green-soft)] grid place-items-center mx-auto">
                  <CheckCircle2 size={28} className="text-[var(--green)]" />
                </div>
                <h3 className="font-news text-[22px] font-semibold mt-4">Message sent</h3>
                <p className="text-[var(--ink-soft)] text-sm mt-2 max-w-sm">
                  Thanks{form.name ? `, ${form.name.split(" ")[0]}` : ""} — we've got your note and will reply to{" "}
                  <span className="font-semibold text-[var(--ink)]">{form.email || "your email"}</span> shortly.
                </p>
                <button onClick={() => { setSent(false); setForm({ name: "", email: "", org: "", topic: "General enquiry", message: "" }); }} className="mt-5 text-sm font-semibold text-[var(--clay-700)]">
